        ...
        tabla.innerHTML += `
            <tr>
                <td class="ps-4">
                    ...
                    <small class="text-muted d-block text-start" style="font-size:0.7rem">ID: ${p.id}</small>
                </td>
                <td class="text-start"><i class="bi bi-geo-alt me-1"></i>${p.ubicacion}</td>
                <td class="text-end pe-4">
                    <button class="btn btn-sm btn-outline-warning border-0" onclick="prepararEdicion('${p.id}')"><i class="bi bi-pencil"></i></button>
                    <button class="btn btn-sm btn-outline-danger border-0" onclick="eliminarPuerta('${p.id}')"><i class="bi bi-trash"></i></button>
                </td>
            </tr>`;
    });
}
            ...
            <div class="col-md-4 mb-4">
                <div class="card card-access h-100 shadow-sm p-3">
                    ...
                    <div class="d-flex justify-content-between align-items-center bg-light p-3 rounded-4">
                        <span class="small fw-bold">${p.estado ? 'CERRAR' : 'ABRIR'}</span>
    ...
    puertas.forEach(p => {
        tabla.innerHTML += `
            <tr class="align-middle">
                ...
                <td class="fw-bold">${p.hora_apertura || '--:--'}</td>
                <td class="fw-bold">${p.hora_cierre || '--:--'}</td>
            </tr>`;
    });
}

// Funciones CRUD (Iguales a las anteriores pero asegurando que los IDs se manejen correctamente)
async function togglePuerta(id, estadoActual) {
    ...
    if (nuevoEstado) {
        ...
        hA.unshift(hora); localStorage.setItem(`lista_A_${id}`, JSON.stringify(hA.slice(0, 5)));
        ...
        hC.unshift(hora); localStorage.setItem(`lista_C_${id}`, JSON.stringify(hC.slice(0, 5)));
